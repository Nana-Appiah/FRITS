...
Public Delegate Sub FooterEvent(row As GridViewRow)
...
Public Class GridViewHelper
	...
	Public Sub New(grd As GridView)
		Me.New(grd, False, SortDirection.Ascending)
	End Sub

	Public Sub New(grd As GridView, useFooterForGeneralSummaries As Boolean)
		Me.New(grd, useFooterForGeneralSummaries, SortDirection.Ascending)
	End Sub

	Public Sub New(grd As GridView, useFooterForGeneralSummaries As Boolean, groupSortDirection As SortDirection)
		Me.mGrid = grd
	...
	Public Function RegisterSummary(column As String, operation As SummaryOperation) As GridViewSummary
		Return Me.RegisterSummary(column, [String].Empty, operation)
	End Function

	Public Function RegisterSummary(column As String, formatString As String, operation As SummaryOperation) As GridViewSummary
	...
	Public Function RegisterSummary(column As String, operation As SummaryOperation, groupName As String) As GridViewSummary
		Return Me.RegisterSummary(column, [String].Empty, operation, groupName)
	End Function

	Public Function RegisterSummary(column As String, formatString As String, operation As SummaryOperation, groupName As String) As GridViewSummary
		If operation = SummaryOperation.[Custom] Then
			Throw New Exception(USE_ADEQUATE_METHOD_TO_REGISTER_THE_SUMMARY)
		End If
	...
	Public Function RegisterSummary(column As String, operation As CustomSummaryOperation, getResult As SummaryResultMethod) As GridViewSummary
		Return RegisterSummary(column, [String].Empty, operation, getResult)
	End Function

	Public Function RegisterSummary(column As String, formatString As String, operation As CustomSummaryOperation, getResult As SummaryResultMethod) As GridViewSummary
		' TO DO: Perform column validation...
	...
	Public Function RegisterSummary(column As String, operation As CustomSummaryOperation, getResult As SummaryResultMethod, groupName As String) As GridViewSummary
		Return RegisterSummary(column, [String].Empty, operation, getResult, groupName)
	End Function

	Public Function RegisterSummary(column As String, formatString As String, operation As CustomSummaryOperation, getResult As SummaryResultMethod, groupName As String) As GridViewSummary
		Dim group As GridViewGroup = mGroups(groupName)
		If group Is Nothing Then
			Throw New Exception([String].Format(GROUP_NOT_FOUND, groupName))
		End If
	...
	Public Function RegisterSummary(s As GridViewSummary) As GridViewSummary
	...
	Public Function RegisterGroup(column As String, auto As Boolean, hideGroupColumns As Boolean) As GridViewGroup
		Dim cfams As String() = New String(0) {column}
		Return RegisterGroup(cfams, auto, hideGroupColumns)
	End Function

	Public Function RegisterGroup(columns As String(), auto As Boolean, hideGroupColumns As Boolean) As GridViewGroup
	...
	Public Function SetSuppressGroup(column As String) As GridViewGroup
		Dim cfams As String() = New String(0) {column}
		Return SetSuppressGroup(cfams)
	End Function

	Public Function SetSuppressGroup(columns As String()) As GridViewGroup
	...
	''' <returns></returns>
	Private Function EvaluateEquals(g As GridViewGroup, dataitem As Object) As Boolean
	...
	Private Function HasAutoSummary(list As List(Of GridViewSummary)) As Boolean
	...
	Private Function GetGroupRowValues(g As GridViewGroup, dataitem As Object) As Object()
	...
	''' <param name="g"></param>
	''' <returns></returns>
	Private Function InsertGridRow(beforeRow As GridViewRow, g As GridViewGroup) As GridViewRow
	...
	Private Sub RowDataBoundHandler(sender As Object, e As GridViewRowEventArgs)
		For Each g As GridViewGroup In mGroups
	...
	Private Sub ProcessGroup(g As GridViewGroup, e As GridViewRowEventArgs)
	...
	Private Function GetFormatedString(preferredFormat As String, secondFormat As String, value As Object) As String
	...
	Private Sub GenerateGroupSummary(g As GridViewGroup, row As GridViewRow)
	...
	Private Sub GenerateGeneralSummaries(e As GridViewRowEventArgs)
		Dim colIndex As Integer
	...
	''' <returns></returns>
	Private Function ResolveCellIndex(row As GridViewRow, colIndex As Integer) As Integer
		Dim cfamspansum As Integer = 0
		Dim realIndex As Integer

		For i As Integer = 0 To row.Cells.Count - 1
			realIndex = i + cfamspansum
			If realIndex = colIndex Then
				Return i
			End If

			If row.Cells(i).ColumnSpan > 1 Then
				cfamspansum = cfamspansum + row.Cells(i).ColumnSpan - 1
			End If
		Next
	...
	Private Function ColumnHasSummary(colindex As Integer, g As GridViewGroup) As Boolean
	...
	Private Function ColumnHasSummary(column As String, g As GridViewGroup) As Boolean
		Dim list As List(Of GridViewSummary)
	...
	Public Function GetRealIndexFromVisibleColumnIndex(visibleIndex As Integer) As Integer
	...
	Public Function GetVisibleColumnIndex(columnName As String) As Integer
		Dim visibles As Integer = 0
		For i As Integer = 0 To mGrid.Columns.Count - 1
	...
	Public Function GetDataFieldName(field As DataControlField) As String
	...
	Public Function GetColumnFormat(colIndex As Integer) As String
		' TO DO: Enable search in HyperLinkField, ButtonField...
	...
	''' <param name="beforeRow"></param>
	''' <returns></returns>
	Public Function InsertGridRow(beforeRow As GridViewRow) As GridViewRow
		Dim visibleColumns As Integer = Me.GetVisibleColumnCount()
    ...
    Public Sub ApplyGroupSort()
        Dim expression = Me.GetSequentialGroupColumns().ToString
        mGrid.Sort(expression, groupSortDir)
    End Sub
