...
#Region " Methods"
    ...
    Private Sub GetData()
        Try
            ...
            Dim orgunit As AROrgUnit
            orgunit = Me.ARConn.GetOrgUnitByID(Me.ObjectID)

            If Not (orgunit Is Nothing) Then

                Try
                    If Not orgunit.ObjectName Is DBNull.Value Then
                        Me.txtName.Text = orgunit.ObjectName
                    End If
                Catch ex As Exception
                End Try

                Try
                    If Not orgunit.ObjectDescription Is DBNull.Value Then
                        Me.txtDesc.Text = orgunit.ObjectDescription
                    End If
                Catch ex As Exception
                End Try
    ...
    Private Sub SaveData()
        Try
            ...
            If Not Me.ObjectID = Nothing Then
                ...
                orgunit.ObjectName = Me.txtName.Text
                orgunit.ObjectDescription = Me.txtDesc.Text
                orgunit.Update()

                'Log into auditing log
                Me.Log(Me.CurrentUser.ObjectID, Me.GetResourceID(Me.ObjectAlias), Me.ResManager.GetString("GroupDetails.Log.Change", Thread.CurrentThread.CurrentUICulture) & " " & orgunit.ObjectName, "FRITS")

                'Display message
                ScriptManager.RegisterStartupScript(Me, Me.GetType(), "jsCall", "alert('Changes saved successfully.');parent.returnValue='reload';", True)

            Else
                orgunit = New AROrgUnit
                orgunit.ARConnection = Me.ARConn
                ...
                If Me.txtName.Text = "" Then
                    'Display message
                    ScriptManager.RegisterStartupScript(Me, Me.GetType(), "jsCall", "alert('" & Me.ResManager.GetString("OrgUnitNew.lblErrorOrgUnitName.Empty", Thread.CurrentThread.CurrentUICulture) & "');", True)
                Else
                    Try
                        'Create new group
                        orgunit.ObjectName = Me.txtName.Text
                        orgunit.ObjectDescription = Me.txtDesc.Text
                        orgunit.ObjectAlias = Me.txtName.Text.Replace(" ", "_")
                        orgunit.InsertNew()

                        'Log into auditing log
                        Me.Log(Me.CurrentUser.ObjectID, Me.GetResourceID(Me.ObjectAlias), Me.ResManager.GetString("GroupNew.Log.InsertNew", Thread.CurrentThread.CurrentUICulture) & " " & orgunit.ObjectAlias, "FRITS")

                        'Display message and close window
                        ScriptManager.RegisterStartupScript(Me, Me.GetType(), "jsCall", "alert('" & Me.ResManager.GetString("OrgUnitNew.lblMessage.Created", Thread.CurrentThread.CurrentUICulture) & "');parent.returnValue='reload';", True)
                    ...
                    Catch exc As Exception
                        'Display message
                        ScriptManager.RegisterStartupScript(Me, Me.GetType(), "jsCall", "alert('" & Me.ResManager.GetString("OrgUnitNew.lblMessage.Error", Thread.CurrentThread.CurrentUICulture) & "');", True)
                        Exit Sub
                    End Try
...
#Region " MemberOf"
    ...
    Private Sub LoadMemeberOfs()
        ...
        rootnode.ImageUrl = "~/Images/Icons/20x20/OrgUnit.gif"
        ...
        If ds.Tables(0).Rows.Count > 0 Then
            ...
            For Each dr As DataRow In ds.Tables(0).Rows
                ...
                If dr("ChildObjectTypeNamespace").ToLower = ARConstants.cUSERGROUP_TYPE_NAMESPACE.ToLower Then
                    ...
                    node.ImageUrl = "~/Images/Icons/16x16/OrgUnit.gif"
...
#End Region
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            ...
            'Set resource id
            Me.ObjectAlias = "frits.utilities.profilemanager.orgunits"
            ...
            If Not Me.Page.IsPostBack Then
                ...
                Else
                    ...
                    ScriptManager.RegisterStartupScript(Me, Me.GetType(), "jsCall", "parent.window.document.title='New Org. Unit';", True)
                End If
